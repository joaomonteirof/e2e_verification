...
def train(lr, l2, beta1, beta2, smoothing, patience, model, emb_size, n_hidden, hidden_size, dropout_prob, epochs, batch_size, valid_batch_size, n_workers, cuda, data_path, valid_data_path, hdf_path, valid_hdf_path, checkpoint_path, softmax, n_classes, pretrained, pretrained_path, max_gnorm, lr_factor, stats, log_dir):
	...
	if hdf_path != 'none':
		transform_train = transforms.Compose([transforms.ToPILImage(), transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip(), transforms.RandomChoice((transforms.RandomRotation(30), transforms.RandomPerspective(p=0.4), transforms.RandomGrayscale(p=0.4))), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])	
		trainset = Loader(hdf_path, transform_train)
	else:
		transform_train = transforms.Compose([transforms.RandomResizedCrop(224), transforms.RandomHorizontalFlip(), transforms.RandomChoice((transforms.RandomRotation(30), transforms.RandomPerspective(p=0.4), transforms.RandomGrayscale(p=0.4))), transforms.Normalize(mean=mean, std=std)])
